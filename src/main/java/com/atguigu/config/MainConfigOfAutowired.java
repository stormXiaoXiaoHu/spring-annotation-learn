 ...
 *      @Resource ：可以和@Autowired 一样实现自动装配功能， 默认是按照组件的名称进行装配
 *                  没有能支持@Primary和没有能支持@Autowired(required = false)
 ...
 *      1） 标注在方法上位置：@Bean+方法参数，参数从容器中获取； 默认不写@AutoWired效果是一样的， 都能自动装配
 *      2） 【标在构造器上】 ： 如果组件只有一个有参构造器， 这个有参构造器的@Autowired可以省略，参数位置的组件可以自动从容器中获取
 *      3） 标在参数位置：
 *
 *  4、 自定义组件想要使用Spring容器底层的一些组件（ApplicationContext、BeanFactory、XXX）
 *       自定义组件实现XXXAware：在创建的时候，会调用接口规定的方法注入相关组件，AWare:
